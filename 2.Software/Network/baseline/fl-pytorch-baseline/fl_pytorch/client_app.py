...
class FlowerClient(NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        train_loss, updates = train(self.net, self.trainloader, self.local_epochs, self.device)
        # train_loss = train_Prox(self.net, self.trainloader, self.local_epochs, self.device)
        # print(f'客户端{self.partition_id}开始训练！')
        # 返回训练完成后的模型权重、训练样本数以及训练损失。
        return get_weights(self.net), len(self.trainloader.dataset), {"train_loss": train_loss, "tau": int(updates)}
