...
def load_best_round_preds(num_rounds: int, num_subjects: int, dirpath: str = "client_preds"):
    ...
    y_preds_list = []
    best_rounds = []
    exclude_subjects = [22, 46]

    for subj_idx in range(num_subjects):
        # # 跳过被排除的被试
        # if subj_idx in exclude_subjects:
        #     continue
        # 支持 partition_id 为整数或其它命名：这里按 subj_idx 当作 partition id 字符串
        pid = str(subj_idx)
        best_acc = -1.0
...
if __name__ == '__main__':
    ...
    # class_names = [f"{0.2 * i + 8:.1f}" for i in range(40)]
