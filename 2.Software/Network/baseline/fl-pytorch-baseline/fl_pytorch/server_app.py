...
from flwr.server.strategy import FedAvg, FedAdam, FedMedian, FedTrimmedAvg
...
class MyFedNova(FedAvg):
    ...
    def configure_fit(
        ...
        self.latest_parameters = parameters_to_send
    ...
    def aggregate_fit(
        ...
        # 如果不接受失败且出现失败，则直接返回 None
        if not self.accept_failures and failures:
            print("aggregate_fit: failures present and accept_failures=False")
            return None, {}

        # 取回并强制转换为 float64
        server_ndarrays = parameters_to_ndarrays(self.latest_parameters)
        # 强制所有 server 参数为浮点，避免后续将 float 加到 int 上导致 casting error
        server_ndarrays = [arr.astype(np.float64, copy=False) for arr in server_ndarrays]

        # 总样本数（用于计算 p_i）
        total_examples = sum([fit_res.num_examples for _, fit_res in results])
        if total_examples == 0:
            print("aggregate_fit: total_examples == 0, returning None")
            return None, {}

        # 收集客户端参数与 tau、num_examples
        client_ndarrays_list = []
        taus = []
        nums = []
        for _, fit_res in results:
            # client parameters -> ndarray list
            client_ndarrays = parameters_to_ndarrays(fit_res.parameters)
            client_ndarrays_list.append(client_ndarrays)

            # client num examples
            n_i = int(fit_res.num_examples)
            nums.append(n_i)

            # 尝试从 metrics 中读取 tau（本地更新步数）
            tau_i = None
            metrics = fit_res.metrics or {}
            # 常见命名的容错
            for key in ("tau", "local_steps", "local_updates", "num_updates", "local_epochs"):
                if key in metrics:
                    try:
                        tau_i = float(metrics[key])
                        break
                    except Exception:
                        pass
            # 回退值（可改为 raise）
            if tau_i is None:
                tau_i = 1.0
                print("aggregate_fit: client did not report tau; defaulting tau=1.0 for one client")

            taus.append(float(tau_i))

        # 计算 p_i = n_i / sum n
        ps = [n_i / total_examples for n_i in nums]

        # 计算 tau_eff = sum_i p_i * tau_i （加权平均的本地步数）
        tau_eff = float(sum(p_i * tau_i for p_i, tau_i in zip(ps, taus)))

        # 检查所有 ndarrays 层数/shape 一致
        num_layers = len(server_ndarrays)
        for i, nw in enumerate(client_ndarrays_list):
            if len(nw) != num_layers:
                raise ValueError(f"Client {i} parameters length != server parameters length")

        # 计算 sum_i p_i * ( (client - server) / tau_i )
        # 初始化为全 0 的 ndarrays（与服务器参数同 shape）
        normalized_sum = [np.zeros_like(arr) for arr in server_ndarrays]

        for client_arrs, p_i, tau_i in zip(client_ndarrays_list, ps, taus):
            # normalized contribution per-layer
            for layer_idx in range(num_layers):
                delta = client_arrs[layer_idx] - server_ndarrays[layer_idx]  # Δ_i
                normalized = delta / float(tau_i)  # Δ_i / τ_i
                normalized_sum[layer_idx] += p_i * normalized  # p_i * (Δ_i / τ_i)

        # aggregated_ndarrays = server + tau_eff * normalized_sum
        aggregated_ndarrays = [
            server_ndarrays[layer_idx] + tau_eff * normalized_sum[layer_idx]
            for layer_idx in range(num_layers)
        ]
